...
def plot_xG_rolling(team, ax, color_for , color_ag ,X_ ,Y_for ,Y_ag):
    ...
    for_ = ax.plot(X_, Y_for, marker = "o", mfc = "white", ms = 4, color = color_for,label='预测mRS等级')
    ag_ = ax.plot(X_, Y_ag, marker = "o", mfc = "white", ms = 4, color = color_ag,label='真实mRS等级')
...
data = pd.read_excel('3ab预测结果与真实结果数据.xlsx')
a=data['3a预测'][:100]
b=data['3b预测'][:100]
ab=data['90天mRS'][:100]
x=pd.Series(range(1,len(ab)+1))
fig = plt.figure(figsize=(5, 2), dpi = 180)
ax = plt.subplot(111)
plot_xG_rolling("90天mRS预测结果（只包含首次影像）", ax, color_for = "#87D8F7", color_ag = "#15366F",X_=x,Y_for=a,Y_ag=ab)
